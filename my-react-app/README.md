...
- updated copy of the variable must be passed in the func
...
if we click the button 5 times within three seconds the value displayed still would be 1 instead of 5.
this happens because setValue is being called asynchronously,
so it is referencing to the current state of value which is 0
...
here we are calling the setValue synchronously i.e,
second setValue call will be made only after the first setValue
call ends which means the second setValue will reference the
updated state of the value variable(done by the first setValue call)
...
- cb can't return promise (so can't make it async)
  but we can use async inside the callback
...
const Example = () => {
  ...
  // this will also fail - call useEffect before return
  useEffect(() => {
    console.log('hello there')
  }, [])
...
const ShortCircuitOverview = () => {
  ...
  return (
    <div>
      ...
      <h4>Falsy OR : {text || 'hello world'}</h4> {/*output : hello world*/}
      <h4>Falsy AND {text && 'hello world'}</h4>
      {/*output : empty string (text variable*/}
      <h4>Truthy OR {name || 'hello world'}</h4> {/*output : susan*/}
      <h4>Truthy AND {name && 'hello world'}</h4> {/*output : hello world*/}
      {codeExample}
    </div>
  )
}
